...
// --- Example presets --------------------------------------------------------
// Ready-made builds loaded from the "Load an example" menu. Each is just the set
// of URL params it differs from the defaults by (the same encoding the "Copy
// link" button produces), so picking one navigates to that config and the page
// re-seeds itself from the URL. Values are plain (un-encoded); URLSearchParams
// handles escaping.
...
// A labelled button that opens a grouped popover of the example builds above.
function ExamplesMenu({ onPick }: { onPick: (p: ExamplePreset) => void }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="preset-menu preset-menu-labelled wb-examples" ref={ref}>
      <button
        type="button"
        className={"preset-labelbtn" + (open ? " open" : "")}
        aria-expanded={open}
        onClick={() => setOpen((o) => !o)}
      >
...
export function WheelBuilding() {
  ...
  // Loading an example just navigates to its config; the page re-seeds from the
  // URL (App remounts it on the hash change — see useHashConfigKey).
  ...
  return (
    <>
      <div className="wb-toolbar">
        <ExamplesMenu onPick={applyExample} />
      </div>
      <Section
        title="Rim"
        info={
